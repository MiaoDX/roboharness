...
Run:
    ...
    MUJOCO_GL=osmesa python examples/demos/g1/lerobot_locomotion.py
...
def generate_html_report(output_dir: Path) -> Path:
    ...
    return _generate(
        output_dir,
        "lerobot_g1",
        title="LeRobot G1 Validation Report",
        accent_color="#2d8cf0",
        summary_html=(
            f"<strong>Model:</strong> Unitree G1 29-DOF ({G1_HF_REPO})"
            "<br/><strong>Controller:</strong> GR00T Balance + Walk (ONNX)"
        ),
        footer_text="Generated by <code>examples/demos/g1/lerobot_locomotion.py --report</code>",
        meshcat_mode="none",
    )
